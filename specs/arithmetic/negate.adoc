...
*Parameters*: - `+a+`: The number to negate

*Return Value*: The arithmetic negation of `+a+`, such that
`+add(a, negate(a)) = 0+`.

*Properties*: - Involutive: `+negate(negate(a)) = a+` - Identity:
`+negate(0) = 0+` - Self-inverse: Applying twice returns original value
- Reversible: Operation can be undone by applying again

*Edge Cases*: - Overflow on minimum integer values is
