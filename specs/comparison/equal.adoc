...
*Properties*: - Reflexive: `+equal(a, a)+` is always `+true+` -
Symmetric: if `+equal(a, b)+` is `+true+`, then `+equal(b, a)+` is
`+true+` - Transitive: if `+equal(a, b)+` and `+equal(b, c)+`, then
`+equal(a, c)+`

*Edge Cases*: - Floating-point precision issues are
implementation-defined - NaN equality (typically `+NaN ≠ NaN+`) is
implementation-defined - Positive and negative zero equality is
