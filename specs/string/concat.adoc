...
*Parameters*: - `+a+`: The first string - `+b+`: The second string

*Return Value*: A new string consisting of all characters from `+a+`
followed by all characters from `+b+`.

*Properties*: - Associative:
`+concat(concat(a, b), c) = concat(a, concat(b, c))+` - Identity
element: `+concat(a, "") = concat("", a) = a+` (empty string) - NOT
commutative: `+concat(a, b) ≠ concat(b, a)+` (in general)

*Edge Cases*: - Concatenating empty strings returns the other string -
