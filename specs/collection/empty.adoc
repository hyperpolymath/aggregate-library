...
*Return Value*: `+true+` if the collection is empty, `+false+`
otherwise.

*Properties*: - Equivalence: `+empty(c) = equal(length(c), 0)+` -
Deterministic: Always returns the same value for the same collection -
Type-independent: Works for any collection type

*Edge Cases*: - Empty collection returns `+true+` - Any non-empty
